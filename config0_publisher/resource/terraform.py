...
class TFCmdOnAWS(TFAppHelper):
    ...
    def _set_src_envfiles_cmd(self,ssm_name=None):
        ...
        base_cmd = f'if [ -f /{self.stateful_dir}/{self.envfile} ]; then cd /{self.stateful_dir}/; set -a; . ./{self.envfile}; set +a'
        ...
        if not ssm_name:
            ...
            ssm_cmd = f'if [ -f /{self.ssm_tmp_dir}/.ssm_value ]; then cd /{self.ssm_tmp_dir}/; set -a; . ./.ssm_value; set +a; env'
