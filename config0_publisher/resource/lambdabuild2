...
from config0_publisher.cloud.aws.lambdabuild2 import LambdaResourceHelper
...
class LambdaParams(TFAwsBaseBuildParams):
    ...
    def get_base_event(self):
        self._set_inputargs()
        lambda_helper = LambdaResourceHelper(s3_output_key=self.s3_output_key,
                                           **self.buildparams)
        return lambda_helper.get_base_event()


class Lambdabuild(LambdaParams):
    # Valid method options
    VALID_METHODS = ["create", "apply", "pre-create", "validate", "check", "destroy"]
    
    def __init__(self, **kwargs):
        self.classname = "Lambdabuild"
        LambdaParams.__init__(self, **kwargs)

        self.tfcmds = TFCmdOnAWS(
            runtime_env="lambda",
        ...
        self.infracost_cmds = TFInfracostHelper(
            runtime_env="lambda",
            envfile="build_env_vars.env",
            binary='infracost',
            version="0.10.39",
    ...
    def _get_build_cmds(self):
        ...
        if self.method in ["create", "apply"]:
            ...
            valid_methods = ", ".join(self.VALID_METHODS)
            raise ValueError(f"Invalid method: {self.method}. Method must be one of: {valid_methods}")
    ...
    def run(self):
        ...
        response = self.stepf.invoke_with_lambda(**event)
        execution_arn = response['executionArn']




Lambdabuild(**cinputargs)
