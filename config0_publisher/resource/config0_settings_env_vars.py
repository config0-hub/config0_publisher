...
class Config0SettingsEnvVarHelper:
    """Helper class for handling Config0 settings and environment variables."""

    def __init__(self):
        """Initialize the helper with empty variables and a logger."""
    ...
    def _set_frm_config0_resource_settings(self, raise_on_error=None):
        """Initialize Config0 resource settings from environment variables."""
        ...
        if resource_runtime_settings_hash:
            try:
                ...
                self.logger.warn(f"Error decoding resource_runtime_settings: {str(e)}")
        ...
        if tf_runtime_settings_hash:
            try:
                ...
                self.logger.warn(f"Error decoding tf_runtime_settings: {str(e)}")

    def _set_tf_runtime(self):
        """Set Terraform runtime configuration."""
        tf_runtime = None
    ...
    def _set_tf_binary_version(self):
        """Parse and set Terraform binary and version."""
        try:
    ...
    def eval_config0_resource_settings(self, method=None):
        """Evaluate Config0 resource settings based on environment variables."""
